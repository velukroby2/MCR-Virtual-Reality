...
try{
  ...
  for(const path of ['/package.json','/render.yaml','/scripts/serve.mjs','/tests/model.test.mjs','/missing.mp4','/.mcr-media-work/Sintel-source.mp4']){assert.equal((await fetch(base+path)).status,404,path);checks++;}
  ...
  assert.match(html,/Sintel.*Blender Foundation/);assert.match(html,/creativecommons\.org\/licenses\/by\/3\.0/);checks+=2;
  console.log(`${checks} served-site checks passed, including the panorama, MP4 byte ranges, credits and DOM bindings.`);
